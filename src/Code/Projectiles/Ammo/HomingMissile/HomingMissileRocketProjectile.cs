...
namespace Ancient.src.Code.Projectiles.Ammo.HomingMissile
{
    internal class HomingMissileRocketProjectile : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            {
                if (SearchForTargets(Main.player[Projectile.owner], out float a, out Vector2 pos)) {
                    Vector2 direction = pos - Projectile.position;
                    direction.Normalize();
                    Projectile.velocity += direction * 1.6f;
        ...
        private bool SearchForTargets(Player owner, out float distanceFromTarget, out Vector2 targetCenter)
        {
            ...
            for (int i = 0; i < Main.maxNPCs; i++)
            {
                ...
                if (npc.CanBeChasedBy())
                {
                    ...
                    bool closeThroughWall = between < 16 * 3f;

                    if (((closest && inRange) || !foundTarget) && (lineOfSight || closeThroughWall))
                    {
                        if (MathUtils.AngleBetween(npc.Center - Projectile.Center, Projectile.velocity) < 1.3f)
                        {
