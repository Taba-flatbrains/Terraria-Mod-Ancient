...
{
    internal class AquaticTotem : ModItem
    {
        ...
        public override void SetDefaults()
        {
            ...
            Item.shoot = ModContent.ProjectileType<AquaticTotemProjectile>(); // This item creates the minion projectile
        }
    ...
    internal class AquaticTotemProjectile : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            Lighting.AddLight(Projectile.Center + visualOffsetY + new Vector2(0, 30), Color.Cyan.ToVector3() * 1.5f);
            if (ticks % 3 == 0)
            {
                Dust.NewDust(Projectile.position + visualOffsetY + new Vector2(-3, 47), 28, 10, ModContent.DustType<TotemHoveringDust>(), SpeedY: -2, newColor: new Color(0.21f, 0.61f, 0.46f));
            }
            ...
            for (int i = 0; i < Main.maxPlayers; i++)
            {
                ...
                {
                    if (Vector2.Distance(player.Center, Projectile.Center) > 16 * 50) { continue; }
                    player.AddBuff(ModContent.BuffType<SlipperyBuff>(), 2);
                }
            }
            ...
            if (ShamansArmuletPlayer.HasItem(Owner) && Main.myPlayer == Projectile.owner && Projectile.damage != 0)
            {
                if (ticks % (60 * 3) == 0)
                {
                    ...
                    if (SearchForTargets(out float a, out Vector2 targetCenter))
                    {
                        ...
                        Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center + new Vector2(0, -10), projectileDirection, ModContent.ProjectileType<FishProjectile>(),
