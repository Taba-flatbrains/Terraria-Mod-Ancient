...
namespace Ancient.src.Code.Tiles
{
    internal class TileUtils
    {
		...
		public static Point16 GetTopLeftTileInMultitile(int x, int y)
        {
            ...
            //Main.NewText(x - frameX, Color.Red);
            //Main.NewText(y - frameY, Color.Red);

            return new Point16(x - frameX, y - frameY); // needs + 1 becuase I always overshoot my texture size by 2 i think 
        }
        ...
        public static bool TryGetTileEntityAs<T>(int i, int j, out T entity) where T : TileEntity
        {
            ...
            Main.NewText("Error: Tile Entity Not Found (TileUtils.cs)");
            entity = null;
            return false;
        }
    }
